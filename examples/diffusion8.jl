...
Random.seed!(42)
n = 40
x0 = ArrayPartition(sort(vcat(randn(n) .- 1, randn(n) .+ 1)))
x0 = ArrayPartition(tan.(range(-pi/2,pi/2,length=2n+3)[2:end-1]))
...
anim = @animate for t in range(tspan...; length=100)
    plot(title="Density", xlabel="position", ylabel="density",
        xlims=(-7,7), ylims=(0,.4), legend=:topleft, size=(1200, 800))
