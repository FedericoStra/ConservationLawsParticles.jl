...
plot!(ssol, vars=1:5:201; color=:blue)
plot!(isol, vars=1:5:201; color=:red)
savefig("traffic-diffusion.png")
...
anim = @animate for t in range(tspan...; step=1/24)
    ...
    plot!(p, repeat(x, inner=(1,2))[1:20:201,:]',
        hcat(zero(min_dens), min_dens)[1:20:201,:]',
        color=:gray, opacity=.25)
    plot_density!(p, ssol(t); color=:blue)
    plot_density!(p, isol(t); color=:red)
end
gif(anim, "traffic-diffusion.gif")
